---
title: "Progress Memo"
subtitle: |
  | Final Project 
  | Foundations of Data Science with R (STAT 359)
author: "YOUR NAME"
date: today

format:
  html:
    toc: true
    embed-resources: true
    
execute:
  echo: false
  warning: false

from: markdown+emoji 
reference-location: margin
citation-location: margin
---

::: {.callout-important}
## Memo format

Like all other submissions for this course, we expect the memo to be neat and well organized. This a template to get you started. 

You should make this document your own. To help with this we have added a date to the yaml above (good whenever writing memos). You should remove this block when making this document your own. You should also remove any instructions or sections that are unused.

:::

::: {.callout-tip icon=false}

## Github Repo Link

To link to your github **repo**sitory, appropriately edit the example link below. Meaning replace `https://your-github-repo-url` with your github repo url. Suggest verifying the link works before submitting.

[https://your-github-repo-url](https://your-github-repo-url)

:::

::: {.callout-warning}
GitHub (free account) cannot store large files. If you try to commit and push a large dataset you will have an ERROR! Any file over 100 MB (100,000 KB) needs to be added to the `.gitignore` file BEFORE committing.

**We may need to do that for files in your final project!**
:::

## Data source

Blah blah blah^[an example of footnote --- [content in margins](https://quarto.org/docs/blog/posts/2022-02-17-advanced-layout/)]

## Prediction Problem

Blah blah blah^[another footnote]

## Data quality & complexity check

Blah blah blah

## Target variable analysis

Blah blah blah

## Potential data issues

Blah blah blah

## Misc

Blah Blah
